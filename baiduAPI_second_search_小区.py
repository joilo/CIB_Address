...
if __name__ == '__main__':
    ...
    mkdir(path)
    city_name_list = ['黄石']
    searched_city = open("./小区地址第三次迭代/城市已爬列表.txt", 'a+', encoding='utf8')
    ...
    for city_name in city_name_list:
        if city_name in searched_city_list:
            ...
            continue
        path = "./小区地址第三次迭代/%s" % (city_name)
        mkdir(path)
        new_txt = open("./小区地址第三次迭代/%s/%s小区地址补充.txt" % (city_name, city_name), 'a+', encoding='utf8')
        searched_list = open("./小区地址第三次迭代/%s/%s已爬列表.txt" % (city_name, city_name), 'a+', encoding='utf8')
        error_list = open("./小区地址第三次迭代/%s/%s报错列表.txt" % (city_name, city_name), 'a+', encoding='utf8')
        filepath = "D:\\working\\python\\XingYe\\CIB_Address\\小区经纬度错误\\小区_二次迭代\\%s_小区.xlsx" % (city_name)
        txt = './小区地址第三次迭代/%s/%s小区地址补充.txt' % (city_name, city_name)
        new_sheet = './小区地址第三次迭代/%s/%s小区地址补充.xlsx' % (city_name, city_name)
        run(filepath)
        print("Successfully saved to " + txt)
        new_txt.close()
        txt_to_xlsx(txt, new_sheet)
        print("Successfully converted to " + new_sheet)
        outfile_road = "./小区地址第三次迭代/%s/%s小区地址补充_切分.txt" % (city_name,city_name)
        outfile_road1 = "./小区地址第三次迭代/%s/%s小区地址补充_切分.xlsx" % (city_name, city_name)
